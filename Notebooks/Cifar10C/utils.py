...
METHODS = ["Uncalibrated", 'TS Source', 'TS Oracle',  'HB', 'Isotonic', 'Beta abm', 'Beta am', 'Beta ab', 'CPCS', 'TransCal', 'HB TopLabel', 'TS', 'Cluster NN', 'Cluster LR', 'Cluster Ensem',
...
def best_rho_calib(model, metric, calib, loss, trainer='4filter_1'):
    df = pd.read_csv(os.path.join('..', 'results', model.lower(), 'cifar10c', loss, 'kfold', 'caligen', calib, trainer, 'kfold_results.csv'))[2:8]
...
def get_res_df(df, trainer, train_f, valid_f, rest_f):
    level = trainer.split('_')[-1]
    result = pd.DataFrame(columns=domains)
    result_std = pd.DataFrame(columns=domains)
    ...
    result['Average'] = result[domains].mean(axis=1)
